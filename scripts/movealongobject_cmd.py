#!/usr/bin/env python
# --------------------------------------
# file:      movealongobject_cmd.py compared to movealongobject.py
# author:    Guannan Cen
# date:      2020-01-06
# brief:     robot moves along the profile to its second end
#            the action is executed by the by cmd_vel (ros topic)
#            Alternatively, the action can be executed by move_base. It's able to move in x and y-direction.
#            If necessary, just change the the line "from movealongobject_cmd import RelativeMovementMiR" in
#            imagebased_grasping_smach4.py to invoke another program.
# --------------------------------------------------
import rospy
from geometry_msgs.msg import Pose, Point, Quaternion, Twist, PoseWithCovariance
import numpy as np
import actionlib
...
class RelativeMovementMiR:
    def __init__(self):
        ...
        self.goal2alongobject_cmd_pub = rospy.Publisher("cmd_vel", Twist, queue_size=1)
        self.odom_pose_sub = rospy.Subscriber("odom", Odometry, self.mir_pose__odom_update)
        self.current_pose_odom = Pose()

    def mir_pose_update(self, pose_msg):
        self.current_pose = pose_msg

    def mir_pose__odom_update(self,pose_odom_msg):
        self.current_pose_odom = pose_odom_msg.pose.pose
    ...
    def RMove_execute(self):
        ...
        if manual_confirmation == 'y':
            # alternative using command velocity
            msg = Twist()
            if displacement[0] == 0.:
                msg.linear.x = 0.0
            else:
                msg.linear.x = 0.1*displacement[0]/abs(displacement[0])
            if displacement[1] == 0.:
                msg.linear.y = 0.0
            else:
                msg.linear.y = 0.1*displacement[1]/abs(displacement[1])    
            msg.linear.z = 0
            msg.angular.x = 0
            msg.angular.y = 0
            msg.angular.z = 0

            last_position = self.current_pose_odom.position
            distance = math.sqrt(displacement[0]*displacement[0]+displacement[1]*displacement[1])
            
            moving_flag = True
            while True:
                self.goal2alongobject_cmd_pub.publish(msg)
                rospy.sleep(0.1)
                deltax = abs(last_position.x-self.current_pose_odom.position.x)
                deltay = abs(last_position.y-self.current_pose_odom.position.y)
                if abs(math.sqrt(deltax*deltax+deltay*deltay)-distance)>0.05:
                    moving_flag = True
                    print(deltax,deltay)
                else:
                    moving_flag = False
